...
class TotalLengthTests(unittest.TestCase):
    ...
    def test_use_length_hints(self):
        """
        Check that length hints are used when use_hints=True

        We're setting different length hints from the number of items
        to confirm that the length hint is checked before manually
        looping over the whole iterable
        """
